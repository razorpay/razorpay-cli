...
var listCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if authorized, _ := cmd.Flags().GetInt("authorized"); cmd.Flags().Changed("authorized") {
			q.Set("authorized", fmt.Sprintf("%d", authorized))
		}
...
func init() {
	...
	listCmd.Flags().Int("authorized", 0, "Filter by payment authorization (1 = authorized payments only)")
